...
public class Solution_1210_Ladder1 {
	static int T = 10;
	static int[][] pos = {{0,-1},{0,1}};
	public static void main(String[] args) throws Exception {
		BufferedReader input = new BufferedReader(new FileReader("text_D4/Solution_1210_Ladder1.txt"));
//		BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
		...
		for(int t=1; t<=10; t++) {
			...
			for(int i=arr.length-1; i>=0; i--) {
				if(idx+pos[0][1]>=0 && arr[i][idx+pos[0][1]]==1) {
					while(idx+pos[0][1]>=0 && arr[i][idx+pos[0][1]]==1) {
						idx--;
					}
				}else if(idx+pos[1][1]<arr.length && arr[i][idx+pos[1][1]]==1) {
					while(idx+pos[1][1]<arr.length && arr[i][idx+pos[1][1]]==1) {
						idx++;
					}
				}
			}
